...
const PremiumOrderTable = () => {
  const { premiumOrder, getPermiumOrder, } = useProductOrderContextProvider()
  useEffect(() => { getPermiumOrder(1) }, [premiumOrder.search, premiumOrder.active_date, premiumOrder.expire_date, premiumOrder.status]);

  // data table page change
  const onPageChange = (page) => {
    getPermiumOrder(page);
  };
  ...
  if (premiumOrder.isLoading) {
    return <div className="text-center">{premiumOrder.error_message}</div>;
  } else {
    return (
      <>
        <DataTable
          columns={columns}
          data={premiumOrder.data.payload}
          pagination
          paginationServer
          paginationComponentOptions={{ noRowsPerPage: true }}
          progressPending={premiumOrder.isLoading}
          paginationTotalRows={premiumOrder.data.pagination?.total_data}
...
export default PremiumOrderTable